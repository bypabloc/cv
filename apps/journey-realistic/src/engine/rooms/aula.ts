...
import { spawnRealisticNpc } from '../npc-gltf-loader'
...
export default function buildAula(ctx: RoomCtx): RoomBuild {
  ...
  // Etapa 1 de journey-npc-realism (docs/specs/journey-npc-realism/): este
  // es el UNICO NPC de la app respaldado por el pipeline MPFB2+Rigify real
  // (`devtools/npc_pipeline` -> `/models/npc-base.glb`) en vez del builder
  // procedural. Candidato ideal: solo usa patrulla + idle/walk, los 2
  // clips que ya existen (sin pose fija ni talk/wave que aun no tienen
  // clip — ver animate.py).
  const estudianteRonda = spawnRealisticNpc({
    skin: '#c98f6a',
