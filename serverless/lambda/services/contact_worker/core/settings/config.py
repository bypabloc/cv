"""Configuracion del Lambda `contact_worker`.
...
El Lambda usa el logger / tracer / metrics de Powertools v3 que vive en
...
class LogMetricType(Enum):
    ...
    # Worker operations (dominio del Lambda contact_worker)
    WORKER_MESSAGE_RECEIVED = 'WORKER_MESSAGE_RECEIVED'
    WORKER_MESSAGE_PROCESSED = 'WORKER_MESSAGE_PROCESSED'
    WORKER_MESSAGE_SKIPPED = 'WORKER_MESSAGE_SKIPPED'
    WORKER_PERSISTENCE_FAILED = 'WORKER_PERSISTENCE_FAILED'
    WORKER_EMAIL_FAILED = 'WORKER_EMAIL_FAILED'


class AppConfig(BaseSettings):
    """Configuracion del Lambda `contact_worker`, cargada de env vars.

    Cada campo anotado se carga de la env var homonima en MAYUSCULAS.
    Los nombres de tablas y los paths SSM los inyecta devtools al
    provisionar; los secretos NUNCA se hardcodean — solo se guarda su
    path SSM (el codigo lee el valor en runtime).
    """
    ...
    # Cache table (Powertools usa @cached para los SSM lookups).
