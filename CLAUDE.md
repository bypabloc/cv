...
| Client env sync | [.claude/rules/client-env-sync.md](.claude/rules/client-env-sync.md) | Sincronizar `docker/env/client/.{env}` a GitHub Environment Variables (NO Secrets) via `python devtools/run.py github_sync --env=<X>`. Catalogo, rotacion de Turnstile sitekey, anti-patrones |
