...
def test_email_code_full_lifecycle_setup_required_login_disable(
    ...
    assert setup.status == 204, (
        f'setup-email-code fallo: {setup.status} {setup.body}'
    )
    ...
    assert req.status == 204, (
        f'set-required email_code fallo: {req.status} {req.body}'
    )
    ...
    assert 'email_code' in methods, f'falta email_code en methods: {start.body}'
    assert 'password' in methods, f'falta password en methods: {start.body}'
    ...
    send = http.post(
        '/auth',
        body=make_body('login', 'send-email-code', temp_token=temp2),
        origin=origin,
    )
    assert send.status == 200, (
        f'send-email-code fallo: {send.status} {send.body}'
    )
    # el harness seedea el hash del code en Neon (kind='login').
    assert environment.seed_code(
        user_id=user_id,
        kind='login',
        plaintext=_LOGIN_CODE,
    ), 'seed_code no actualizo ninguna fila de login code'

    vc = http.post(
        '/auth',
        body=make_body(
            'login', 'verify-code', code=_LOGIN_CODE, temp_token=temp2
        ),
        origin=origin,
    )
    ...
    assert field(vc.body, 'access_token'), (
        f'falta access_token final: {vc.body}'
    )
    assert field(vc.body, 'refresh_token'), (
        f'falta refresh_token final: {vc.body}'
    )
