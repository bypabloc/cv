...
│       ├── services/     # los 10 Lambdas (analytics, auth, contact_form,
│       │                 #   cv, db, send_email, tracking_pixel,
│       │                 #   tracking_writer, users)
...
| Schema PostgreSQL unificado | [docs/diagrams/db-er.mmd](docs/diagrams/db-er.mmd) | Schema relacional unico de Neon en `serverless/lambda/shared/db/`: 35 tablas (CV + datos del visitante) modeladas en SQLAlchemy 2.x, gestionadas por un solo Alembic. La Lambda `db` corre las migraciones. Los Lambdas `contact_form` y `tracking_writer` usan el ORM para la replica analitica a Neon. La DB es la FUENTE DE VERDAD del CV (editada via la operation `content` del Lambda `cv` desde el admin); el command `seed` de la Lambda `db` es el RESTORE desde el snapshot S3 semanal (`devtools db_export` + workflow db-backup.yml) |
...
| Config de Lambdas (memoria/timeout) | [.claude/rules/lambda-config.md](.claude/rules/lambda-config.md) | Antes de crear/editar un `manifest.yaml` o diagnosticar un 502/timeout de cold start. **PISO de memoria: `memory: 1024` en TODO manifest** (politica 2026-06-10; NUNCA menos, aunque la medicion diga que cabe). Subir por encima de 1024 requiere medicion documentada en el comentario. El cold start se ataca con imports lazy (PEP 562 en subpaquetes shared con deps pesadas como fido2), no con memoria. memoria == CPU; el timeout cubre el cold SIN SnapStart restore (30-60 segun Lambda) |
...
Skills consolidadas para el backend del portfolio: 10 Lambdas Python 3.13
(analytics, auth, contact_form, cv, db, send_email,
tracking_pixel, tracking_writer, users) detras de API Gateway REST + rate-limit per-IP con DynamoDB (sin
