...
def test_webauthn_full_lifecycle_register_required_login_disable_delete(
    ...
    assert req.status == 204, (
        f'webauthn set-required fallo: {req.status} {req.body}'
    )
    ...
    assert 'webauthn' in methods, f'falta webauthn en methods: {start.body}'
    ...
    dis = http.post(
        '/auth',
        body=make_body('webauthn', 'disable', credential_id=record_id),
        origin=origin,
        bearer=access,
    )
    ...
    ena = http.post(
        '/auth',
        body=make_body('webauthn', 'enable', credential_id=record_id),
        origin=origin,
        bearer=access,
    )
    ...
    deleted = http.post(
        '/auth',
        body=make_body(
            'webauthn', 'delete-credential', credential_id=record_id
        ),
        origin=origin,
        bearer=access,
    )
