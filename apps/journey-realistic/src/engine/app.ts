...
import { getLiveGltfNpcObjects } from './npc-gltf-loader'
import { createNpcOutlineComposer } from './npc-outline'
...
export async function startJourney(opts: StartOptions): Promise<JourneyHandle> {
  ...
  // autoReset=false: el composer del NPC outline (RenderPass + OutlinePass +
  // OutputPass) dispara varios renderer.render() internos por frame; con
  // autoReset=true cada uno reinicia info.render.calls, dejando solo el
  // ULTIMO pase contado. El reset manual UNA vez por frame (ver frame())
  // acumula el total real (necesario para medir AC-8).
  ...
  // Contorno de NPCs GLTF (journey-npc-realism, AC-7): screen-space via
  // OutlinePass, unico camino compatible con SkinnedMesh deformado (el
  // inverted-hull de toon.ts asume geometria estatica). Solo se usa el
  // composer cuando hay al menos un NPC realista vivo en la sala — sin
  // NPCs GLTF, el frame renderiza igual que antes (renderer.render plano).
  const npcOutline = createNpcOutlineComposer({
    renderer,
    scene,
    camera,
  ...
  function onResize(): void {
    ...
    renderer.setSize(container.clientWidth, container.clientHeight)
    npcOutline.setSize(container.clientWidth, container.clientHeight)
  }
  ...
  function frame(): void {
    ...
    sfx.update(dt)
    // renderer.info.autoReset resetea los contadores en CADA
    // renderer.render() interno — el composer del NPC outline hace varios
    // (RenderPass, mascara de OutlinePass, OutputPass): con autoReset=true,
    // renderer.info tras el frame solo reflejaria el ULTIMO pase (el quad
    // fullscreen de OutputPass, calls=1). Reset manual UNA vez por frame
    // acumula el total real de draw calls (necesario para medir AC-8).
    renderer.info.reset()
    const gltfNpcs = getLiveGltfNpcObjects()
    if (gltfNpcs.length > 0) {
      npcOutline.setOutlinedObjects([...gltfNpcs])
  ...
  if (import.meta.env.DEV) {
    ...
    const w = window as Window & {
      __journeyDebug?: {
        ...
        /** Roots de NPCs GLTF vivos (journey-npc-realism) — posiciones reales para smokes. */
        gltfNpcs(): readonly { x: number; y: number; z: number }[]
        /** `pos` de controls.ts es la fuente de verdad (pisa player.group.position cada frame): mutar la posicion a mano no alcanza, hay que usar esto. */
    ...
    w.__journeyDebug = {
      ...
      info: renderer.info,
      gltfNpcs: () =>
        getLiveGltfNpcObjects().map((obj) =>
          obj.getWorldPosition(new Vector3()),
        ),
      teleport: (x, z) => controls.teleport(x, z),
      scene,
      camera,
    }
  }
