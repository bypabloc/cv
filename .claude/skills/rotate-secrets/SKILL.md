...
description: >
  ...
  --cloudflare-account-id). Resolves the 2 Cloudflare Turnstile widgets
  (dev/prod), reads or rotates the widget secret, and updates server
  + client env files (TURNSTILE_SECRET_KEY + PUBLIC_TURNSTILE_SITEKEY).
  Common flags: --dry-run, --rotate, --force, --envs=dev,prod. NOTE: the
...
Configura los 2 widgets Cloudflare Turnstile que cubren los entornos del
portfolio:
