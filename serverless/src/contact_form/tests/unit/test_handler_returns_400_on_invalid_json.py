"""Handler — body con JSON malformado devuelve HTTP 400.

Given un evento API Gateway cuyo body no es JSON valido,
When lambda_handler procesa el evento,
Then devuelve HTTP 400 con code INVALID_JSON (error_response).
"""

import json

import pytest

from tests.unit._helpers import api_gw_event, lambda_context

pytestmark = pytest.mark.unit


def test_handler_returns_400_on_invalid_json(contact_form_aws):
    import handler

    # Arrange
    event = api_gw_event(body='{not valid json', ip='203.0.113.31')

    # Act
    response = handler.lambda_handler(event, lambda_context())

    # Assert
    assert response['statusCode'] == 400
    body = json.loads(response['body'])
    assert body['code'] == 'INVALID_JSON'
