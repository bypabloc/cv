"""Lambda `auth_email_worker` — consume SQS `portfolio-auth-email-${stage}`.

Entrypoint del worker. Itera los Records del evento SQS y delega cada
mensaje al controller `email/process` via `run_controller`. Devuelve
`batchItemFailures` para los items que fallaron (SES throttle, render
template, audit insert), de modo que SQS los reintenta hasta
`max_receive_count=3` antes de mandarlos a la DLQ.

El handler es delgado: NO contiene logica de negocio (que vive en
`core/services/`). Su unico trabajo es:
...
def lambda_handler(event: dict[str, Any], _context: Any) -> dict[str, Any]:
    """Procesa un batch SQS (batch_size=5 en este worker).
    ...
    for record in records:
        ...
        try:
            ...
            logger.exception(
                'failed to process auth email message',
                extra={
                    'message_id': message_id,
                    'error': str(exc),
                },
