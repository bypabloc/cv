"""
Given un evento valido de contacts/by-status con JWT y service mockeado,
...
from controllers.contacts.by_status import ByStatus


def test_contacts_by_status_controller_when_valid_then_calls_service(
    mocker, mock_require_auth, mock_check_or_raise
):
    # Arrange
    expected = {'items': [{'status': 'new', 'count': 5, 'pct': 100.0}]}
    spy = mocker.patch.object(
        contacts_service, 'by_status', return_value=expected
    )
    event = {
        'from': '2026-04-27',
        'to': '2026-05-27',
