"""@module shared.lambda_kit.event_model — modelo Pydantic del evento.

Valida la estructura del evento del estandar lambda-controller
(`{operation, action, data}`) y resuelve el controller correspondiente.

Era identico en los 4 Lambdas del backend. Se unifico aca como una
FACTORY: `build_event_model(operations)` devuelve una clase
`EventModel` ligada al mapa `OPERATIONS` de ese Lambda. Asi el modelo es
generico pero cada Lambda lo construye con SU `OPERATIONS` sin que
`lambda_kit` dependa de ningun Lambda.
"""
...
def build_event_model(
    ...
    """Construye la clase `EventModel` ligada al `OPERATIONS` del Lambda.

    Parameters
    ----------
    operations : dict[str, dict[str, Any]]
        Mapa `OPERATIONS` del Lambda (codename -> controller + arn_key).

    Returns
    -------
    type[BaseModel]
        Una clase `EventModel` cuyo `validate_event` resuelve los
        controllers contra `operations`.
    """
    ...
    class EventModel(BaseModel):
        ...
        controller_info: Any = Field(
            ...,
            description=(
                'Resultado de import_controller (controller resuelto).'
            ),
        )
        ...
        def validate_event(cls, event: dict[str, Any]) -> EventModel:
            ...
            if 'operation' not in event:
                raise ValueError(
                    'No se especifico la operacion a ejecutar'
                )
            ...
            controller_result = import_controller(
                operation_name, action_name, operations
            )
